...
# 例:     bash do_oci_user_grant_admin.sh tanaka.taro
#         bash do_oci_user_grant_admin.sh tanaka.taro prod

LOG="/home/hijiri/claude2026001/do_oci_user_grant_admin_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOG") 2>&1
echo "=== ログ出力先: $LOG ==="
...
# administrator グループ所属確認（既に所属していれば終了）
...
# ─────────────────────────────────────────
# [4/4] administrator グループに追加
# ─────────────────────────────────────────
echo "=== [4/4] '${ADMIN_GROUP}' グループへ追加 ==="
...
echo "  ユーザー名 : ${USERNAME}"
echo "  所属グループ: ${READONLY_GROUP}, ${ADMIN_GROUP}"
echo "========================================"
