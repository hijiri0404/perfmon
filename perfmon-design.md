...
/opt/perfmon/
  ...
  log/
    ...
    conftrack/
      .conftrack_state     # ステートファイル（最終実行日付を管理）
      master_YYYYMMDD.txt  # 月次マスタースナップショット（毎月1日・インストール時）
      diff_YYYYMMDD.txt    # 日次差分レポート（マスターとの unified diff）
      install.log          # RPM %post 実行ログ
/etc/perfmon/
  perfmon.conf             # 設定ファイル
/usr/bin/
...
| RETENTION_DAYS | 31 | ログ保持日数 |
| LOG_DIR | /opt/perfmon/log | ログ出力先 |
| CONFTRACK_ENABLED | yes | 設定ファイル変更追跡の有効/無効 |
| CONFTRACK_DIRS | /etc /var/spool/cron /root /usr/lib/systemd/system | 追跡対象ディレクトリ（スペース区切り） |
| CONFTRACK_TIME | 02:00 | マスター生成・差分チェックの実行時刻 (HH:MM) |
| CONFTRACK_MAX_FILE_SIZE | 524288 | スキップするファイルサイズ上限 (bytes)。デフォルト 512KB |
| CONFTRACK_EXCLUDE_PATHS | /etc/pki:/etc/ssl/certs:... | 追跡除外パス（コロン区切り、glob可） |
| CONFTRACK_METADATA_ONLY_PATHS | /etc/shadow:/etc/gshadow | 内容非記録・メタデータ（サイズ/mtime/sha256）のみ記録するパス |
...
## conftrack — システム設定ファイル変更追跡（v1.4.0〜）

### 概要

`/etc` 等のシステム設定ファイルを定期スキャンし、変更を検知して記録する機能。
`perfmon-conftrack.sh` が `perfmon-collector.sh` に dot-source で取り込まれ、メインループから `conftrack_check()` が毎 60 秒ごとに呼ばれる。

### 動作タイミング

| タイミング | 処理 |
|---|---|
| RPM インストール直後 | `%post` スクリプトがバックグラウンドで `conftrack_init` を呼び出し初回マスターを生成 |
| 毎月 1 日 `CONFTRACK_TIME` 以降 | 月次マスタースナップショット生成（`master_YYYYMMDD.txt`） |
| 毎月 2 日〜末日 `CONFTRACK_TIME` 以降 | 日次差分チェック（`diff_YYYYMMDD.txt`）。当月のマスターと比較 |

`CONFTRACK_TIME` のデフォルトは `02:00`。設定変更後は `systemctl restart perfmon` で反映。

### マスターファイル形式（master_YYYYMMDD.txt）

```
###CONFTRACK_MASTER###
generated=2026-02-01 02:00:00
hostname=webserver01
conftrack_dirs=/etc /var/spool/cron /root /usr/lib/systemd/system
perfmon_version=1.4.0

###FILE### /etc/hostname
size=20
mtime=2026-01-10 11:00:00
sha256=abc123...
---CONTENT---
webserver01.example.com
---END---

###FILE_SKIPPED### /etc/shadow
reason=metadata_only

###FILE_SKIPPED### /etc/firmware/bigfile.bin
reason=binary

###CONFTRACK_MASTER_END###
total_captured=142
total_skipped=18
```

- `---CONTENT---` / `---END---` デリミタで各ファイルの内容を囲む
- バイナリファイルは `reason=binary` でスキップ
- `CONFTRACK_MAX_FILE_SIZE` 超過は `reason=exceeds_max_size` でスキップ
- `CONFTRACK_METADATA_ONLY_PATHS` に指定されたパスは `reason=metadata_only` として sha256/mtime のみ記録（`/etc/shadow` 等の機密ファイル向け）

### 差分ファイル形式（diff_YYYYMMDD.txt）

```
###CONFTRACK_DIFF###
generated=2026-02-21 02:00:05
master_date=20260201
master_file=master_20260201.txt

###NEW### /etc/cron.d/new-job
---CONTENT---
* * * * * root /usr/local/bin/job.sh
---END---

###DELETED### /etc/yum.repos.d/old-repo.repo

###MODIFIED### /etc/ssh/sshd_config
---DIFF---
--- master_20260201 /etc/ssh/sshd_config
...
###CONFTRACK_DIFF_END###
new=1 deleted=1 modified=1 unchanged=141
```
...
| ファイル種別 | 保持期間 |
|---|---|
| `diff_*.txt` | `RETENTION_DAYS` 日（デフォルト 31 日） |
| `master_*.txt` | 90 日 |

### セキュリティ設計

- `/etc/shadow`, `/etc/gshadow` はデフォルトでメタデータのみ記録（内容・sha256 は記録しない）
- バイナリファイル・証明書類（`/etc/pki`, `/etc/ssl/certs`）はデフォルト除外
- SSH ホスト鍵（`/etc/ssh/ssh_host_*`）はデフォルト除外

### ロック機構

`conftrack_generate_master` 実行中は `mkdir` によるディレクトリロック（`.conftrack_master.lock`）を使用。これにより `%post` バックグラウンド実行とデーモン起動の競合を防ぐ。ロックは `trap RETURN` で自動解放。残留ロックが存在する場合は異常終了の証拠（手動で `rmdir` して削除）。

### ステートファイル（.conftrack_state）

```
LAST_MASTER_DATE=20260201
LAST_DIFF_DATE=20260221
```

tmpfile → `mv` のアトミック書き込みで破損防止。
